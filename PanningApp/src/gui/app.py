...
class AudioPanningApp:
    def __init__(self, master):
        ...
        # Audio State
        self.processed_buffer = None # <--- NEW: Stores pre-calculated audio
    ...
    def load_audio_file(self, filename):
        ...
        try:
            self.audio_samples, self.sample_rate = sf.read(self.audio_file, dtype='float32')
        except Exception as e:
    ...
    def get_virtual_source_position(self, pan_value, y_distance):
        x = pan_value
        return np.array([x, y_distance])

    def play_audio(self):
        """Pre-processes audio with current gains and starts playback."""
        ...
        # --- FIX: PRE-PROCESS AUDIO HERE ---
        # We apply the gains to the whole array BEFORE streaming.
        # This prevents the "fade-in" artifact.
        left_ch = self.audio_samples * self.left_gain
        right_ch = self.audio_samples * self.right_gain
        
        # Stack into a (N, 2) stereo array
    ...
    def audio_callback(self, outdata, frames, time, status):
        """Stream the pre-processed buffer."""
        if status:
    ...
    def plot_current_audio(self):
        # Fallback to calculating chunk for visualization
